...
def run(): 
    ...
    if data and isinstance(data[0], list):
        ...
        if selected_spk:
            ...
            def buat_pdf_essence(df, nomor_spk, tanggal_spk):
                pdf = FPDF("L", "mm", "A4")
                ...
                pdf.cell(width_col, 8, "Operator Essence", ln=0, align="C")
                pdf.cell(page_width - 2 * width_col, 8, "", ln=0)
                pdf.cell(width_col, 8, "Ka. Sie Essence", ln=1, align="C")

                # Spasi kosong untuk tanda tangan manual
                pdf.ln(20)

                pdf.cell(width_col, 8, "(                             )", ln=0, align="C")
                pdf.cell(page_width - 2 * width_col, 8, "", ln=0)
                pdf.cell(width_col, 8, "(                             )", ln=1, align="C")


                return pdf.output(dest="S").encode("latin-1")

            pdf_bytes = buat_pdf_essence(grouped, selected_spk, tanggal_spk)
            st.download_button(
                label="📥 Download PDF",
                data=pdf_bytes,
