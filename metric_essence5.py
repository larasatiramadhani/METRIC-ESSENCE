...
def run():
    ...
    st.session_state.setdefault("form_tanggal_hc", date.today())
    options_pr = get_options(APPS_SCRIPT_URL_PR)
    options_es = get_options(APPS_SCRIPT_URL_ES)

    defaults = {
        "form_nospk_pr": "",
        "form_nospk_es": generate_spk_number(st.session_state["form_tanggal_hc"]), 
        "form_tanggal_hc": st.session_state["form_tanggal_hc"], 
        "form_jenisproduk": "", 
        "form_item" : "",
        "form_komponen" : "",
        "form_rencana_total_pemakaian": "",
        "form_flv" : "",
        "form_rencana_flv" : "",
        "form_realisasi_total_pemakaian": "",
        "form_realisasi_flv": "" ,
        "form_keterangan" : "",
    }
    ...
    def filter_dataframe(df):
        ...
        """
        modify = st.checkbox("Tambah Filter",key='form_modify_hc')
    ...
    overview(key_prefix="overview_hc")
    ...
    if "form_tanggal_hc" not in st.session_state:
        st.session_state["form_tanggal_hc"] = datetime.date.today()

    if "form_nospk_pr" not in st.session_state:
        st.session_state["form_nospk_pr"] = ""
    col1, col2 , col3 = st.columns(3)  
    with col1:
        tanggal = st.date_input("Tanggal Input", value=st.session_state.get("form_tanggal_hc"), key="form_tanggal_hc")
        if tanggal != st.session_state["form_tanggal_hc"]:
            st.session_state["form_tanggal_hc"] = tanggal
    ...
    with col3:
        spk_es = st.text_input("No. SPK Essence", value=generate_spk_number(st.session_state["form_tanggal_hc"]), key="form_nospk_es", disabled=True)
    ...
    if spk_pr:
        ...
        try:
            ...
            st.session_state["form_produk"] = jenis_produk_pr 
            ...
            if not casing_options:
                ...
                st.stop()
            if "form_casing" not in st.session_state or st.session_state["form_casing"] not in casing_options:
                st.session_state["form_casing"] = casing_options[0]

            casing_terpilih = st.selectbox("Pilih Casing", casing_options, key="form_casing")
            ...
            """, unsafe_allow_html=True)
                ...
                for komponen in komponen_list:
                    ...
                    quantity =st.session_state["form_quantity"] = selected_komponen[4] 
                    rasio = st.session_state["form_rasio"] = selected_komponen[5]
                    total_based_flv = st.session_state["form_total_based_flv"] = selected_komponen[6]
                    ...
                    for i in range(0, len(flavour_list), 4):
                        ...
                        for j, flavour in enumerate(current_row):
                            try:
                                ...
                                total_rencana= round(filler_batch * total_based_flv, 4)
                    ...
                    for i in range(0, len(flavour_list), 4):
                        ...
                        for j, flavour in enumerate(current_row):
                            try:
                                ...
                                nama = flavour["nama"]
                                persen = float(str(flavour["persen"]).replace(",", "."))
                                total_rencana = ((outputKg * rasio) / quantity)*total_based_flv
    ...
    ################## MULAI DARI SINI ELSE ############   
                ...
                if "form_komponen_rencana" not in st.session_state or st.session_state["form_komponen_rencana"] not in komponen_options:
                    st.session_state["form_komponen_rencana"] = komponen_options[0]

                komponen = st.selectbox("Pilih Komponen", komponen_options, key="form_komponen_rencana")
            ...
            if casing_terpilih.strip().upper() in ['FILLER', 'FILLER FOR 03', 'FILLER FOR 07', 'CORE','STRIPE']:
                ...
                for komponen in komponen_list:
                    ...
                    total_realisasi = st.number_input(
                        ...
                        key=f"form_total_realisasi_{komponen}"
                    )
    ...
    ################## MULAI DARI SINI ELSE REALISASI ############   
                ...
                selected_komponen = next(
                        ...
                        and jenis_produk_pr.lower() in row[0].lower()
                        and row[2] == casing_terpilih
                        and row[3] == st.session_state["form_komponen_rencana"]
                    )
                ...
                # Ambil flavour list dari fungsi baru
                flavour_list = filter_flavour(data_es, jenis_produk_pr, st.session_state["form_komponen_rencana"])

                # Tampilkan header
                st.markdown(f"<h3 style='text-align: center;'>🔸 Kebutuhan Flavour untuk Komponen: {st.session_state["form_komponen_rencana"]}</h3>", unsafe_allow_html=True)
                ...
                total_realisasi = st.number_input(
                        f"Total Realisasi untuk {st.session_state["form_komponen_rencana"]}",
                        key=f"form_total_realisasi_{st.session_state["form_komponen_rencana"]}"
                    )
                if total_realisasi > 0:
                    # Tampilkan dulu flavour-nya
                    for i in range(0, len(flavour_list), 4):
    ...
    ############################################# SUBMIT DATA ###############################################################################
        ...
        st.write("")
        submit_button = st.button("💾 Simpan Data", use_container_width=True, key='form_submit_data')
